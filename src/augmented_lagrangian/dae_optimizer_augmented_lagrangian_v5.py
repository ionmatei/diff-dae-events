...
class DAEOptimizerAugmentedLagrangianV4(DAEOptimizer):
    """
    ...
    def __init__(
        ...
        if self.verbose:
            print(f"Augmented Lagrangian V4 Initialized (Method: {self.method})")
    ...
    def optimize(
        ...
        for k in range(n_iterations):
            ...
            # Solve DAE
            # Note: Re-instantiating DAESolver might be expensive if it recompiles. 
            # Ideally we reuse a solver instance if possible, but for correctness with current class structure:
            solver_step = DAESolver(dae_data_curr, verbose=False)
            
            try:
                # Use robust tolerances from config or arguments
                ...
                if w_step.shape[0] == len(t_array):
                    # Update w and convert to JAX array
                    w = jnp.array(w_step)
                else:
            ...
            # 3. Multiplier Update (Dual Ascent) - REPLACED WITH EXACT ADJOINT
             # Sanity check: replace iterative update with exact adjoint solution
            
            # Compute dL/dy (gradient of loss w.r.t states)
            # We need to compute it efficiently. self.jac has a helper.
            # Convert w to time-major if needed (it is (N, n))
            
            # p_all_iter is already set above
            ...
            # Scale by 1/N if using mean loss (match optimizer logic)
            if self.loss_type == 'mean':
                 n_outputs = y_target.shape[1]
                 n_time = len(t_array)
                 # Note: self.jac.trajectory_loss_gradient_analytical might already handle this?
                 # Checking valid implementation: it usually returns sum grad.
                 # If loss is mean, we divide. Use total elements N_total = N * n_outputs
                 dL_dy = dL_dy / (n_time * n_outputs)
            
            # Exclude initial condition (adjoint is for steps 1..N)
            ...
            # Compile this call if slow, but for now just call the method (it uses vmap inside)
            ...
            # Update eta with exact solution
            eta_new = lambda_adjoint
            
            # eta^{k+1/2} = eta^k + mu * R  <-- OLD
            # eta_new = eta + mu * residuals <-- OLD
