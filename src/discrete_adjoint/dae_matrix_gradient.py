...
class DAEMatrixGradient:
    ...
    def _make_residual_fn(self, structure):
        dims = self.dims
        funcs = self.funcs
    ...
    def _make_loss_fn(self, structure):
        dims = self.dims
        n_x, n_z, n_p = dims
        n_w = n_x + n_z

        def loss_fn(W_flat, p, grid_taus, target_times, target_data, t_final_val, blend_sharpness=150.0, soft_interp=True):
            ...
            def predict_at_t(t_q):
                ...
                for i in range(len(segments_t)):
                    ...
                    if soft_interp:
                         # Implement Gaussian soft interpolation matching DAEPaddedGradient
                         # Adaptive bandwidth
                         t_first = ts[0]
                         t_last = ts[-1]
                         n_pts_seg = len(ts)
                         dt_typ = jnp.maximum((t_last - t_first) / jnp.maximum(n_pts_seg - 1, 1), 1e-12)
                         sigma = 2.0 * dt_typ
    ...
    def _get_gradient_kernel(self, structure):
        ...
        residual_fn = self._make_residual_fn(structure)
        loss_fn = self._make_loss_fn(structure)
        ...
        def compute_grads(W_flat, p_val, grid_taus, target_times, target_data, t_final_val, blend_sharpness, soft_interp):
            ...
            # 2. Loss Gradients (Value and Grad)
            loss_val, dL_dW = jax.value_and_grad(loss_fn, argnums=0)(W_flat, p_val, grid_taus, target_times, target_data, t_final_val, blend_sharpness, soft_interp)
    ...
    def compute_total_gradient(self, sol, p_val, target_times, target_data, blend_sharpness=150.0, soft_interp=True):
        # 1. Pack
        ...
        # 3. Compute
        loss_val, grad_p = kernel(W_flat, p_val, grid_taus_tuple, target_times, target_data, t_final, blend_sharpness, soft_interp)
        
        return grad_p

    def get_internals(self, sol):
        """Expose residual and loss functions for testing."""
        W_flat, structure, grid_taus_list = self.pack_solution(sol)
        return self._make_residual_fn(structure), self._make_loss_fn(structure), W_flat, tuple(grid_taus_list), structure
    ...
    def optimize_adam(self, solver, p_init, opt_param_indices, target_times, target_data,
                      ...
                      blend_sharpness=150.0, print_every=10, soft_interp=True, **kwargs):
