...
def predict_trajectory_kernel_padded_linear(
    ...
    No sigmoid blending - concatenates valid segments and interpolates directly.

    The key insight is that after resampling, each segment has uniform time points.
    We concatenate all valid segments into a single timeline and interpolate.
    At event boundaries, we use the POST-event value (state after reset).
    ...
    # For invalid segments, times are 0 which won't match queries
    flat_t = pad_t.reshape(-1)  # (max_segments * max_points,)
    ...
    # For invalid segments, set times to infinity so they don't interfere
    # with searchsorted
    flat_t_masked = jnp.where(point_mask > 0.5, flat_t, jnp.inf)

    # Sort by time to get proper ordering (segments are already in order,
    # but we need to handle overlapping boundaries correctly)
    # Actually, segments are stored sequentially, so we can just use the flat array
    ...
    def predict_single_time(t_q):
        ...
        def find_in_segment(carry, seg_data):
            ...
            in_range = (t_q >= t_start) & (t_q <= t_end) & (is_valid > 0.5)
        ...
        seg_data = (pad_t, pad_x, pad_z, seg_mask)
...
class DAEOptimizerImplicitAdjoint:
    ...
    def _resample_trajectory(self, aug_sol: AugmentedSolution):
        """
    ...
    @partial(jit, static_argnames=['self'])
    def _compute_loss_and_grad_padded_linear_jit(self, pad_t, pad_x, pad_z, seg_mask,
                                                  target_times, target_outputs, p_all):
        """JIT-stable loss and gradient computation with linear interpolation (matches PyTorch)."""
        def loss_fn(aug_t, aug_x, aug_z):
            y_preds = predict_trajectory_kernel_padded_linear(
                aug_t, aug_x, aug_z, seg_mask,
                target_times, p_all, self._eval_h, self.n_outputs,
                self.max_segments, self.max_points_per_seg, self.n_alg
            )
            return jnp.mean((y_preds - target_outputs)**2)

        loss, vjp_fn = jax.vjp(loss_fn, pad_t, pad_x, pad_z)
        grads = vjp_fn(1.0)
        grad_t, grad_x, grad_z = grads
        # Return zeros for grad_tau since linear method doesn't use event times
        grad_tau = jnp.zeros(self.max_segments)
        return loss, grad_x, grad_z, grad_tau
    ...
    def optimization_step(self, t_span, target_times, target_outputs, p_opt, ncp=200, blend_sharpness=None):
        ...
        n_segments_jit = min(n_segments_actual, self.max_segments)
