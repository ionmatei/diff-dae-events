...
class DAEOptimizerExplicitAdjoint:
    ...
    def _compile_jax_functions(self):
        ...
        def make_namespace(t, x, z, p_all):
            """Build evaluation namespace with JAX arrays."""
            ns = {
        ...
        self._make_namespace = make_namespace

        # f(t, x, z, p) - differential equations
        def eval_f_jax(t, x, z, p_all):
            ns = make_namespace(t, x, z, p_all)
            return jnp.array([eval(expr, ns) for expr in self.solver.f_funcs])

        self._eval_f = eval_f_jax

        # g(t, x, z, p) - algebraic equations
        def eval_g_jax(t, x, z, p_all):
            ns = make_namespace(t, x, z, p_all)
            if not self.solver.g_funcs:
                return jnp.array([])
            return jnp.array([eval(expr, ns) for expr in self.solver.g_funcs])

        self._eval_g = eval_g_jax

        # Zero-crossing functions
        def eval_zc_jax(t, x, z, p_all):
            ns = make_namespace(t, x, z, p_all)
            if not self.solver.zc_funcs:
                return jnp.array([0.0])
            return jnp.array([eval(expr, ns) for expr in self.solver.zc_funcs])

        self._eval_zc = eval_zc_jax

        # Jump map
        def eval_jump_jax(x_pre, z_pre, tau, p_all, event_idx):
            """Compute x_post from x_pre via reinit equation."""
            import re
            x_post = x_pre.copy()
            ...
            ns = make_namespace(tau, x_pre, z_pre, p_all)
            ...
            # Solve linear equation via two-point evaluation
            ns[var_name] = 0.0
            val_at_0 = eval(reinit_modified, ns)
            ns[var_name] = 1.0
        ...
        def eval_h_jax(t, x, z, p_all):
            """Evaluate output function h(t, x, z, p)."""
            # If h_funcs contains state names (default), just return x
            if self.h_funcs == self.solver.state_names:
                return x

            ns = make_namespace(t, x, z, p_all)
            return jnp.array([eval(expr, ns) for expr in self.h_funcs])

        self._eval_h = eval_h_jax
