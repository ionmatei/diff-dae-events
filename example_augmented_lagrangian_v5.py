"""
Example demonstrating Augmented Lagrangian V5 for DAE parameter estimation.

This uses the optimizer defined in `src/augmented_lagrangian/dae_optimizer_augmented_lagrangian_v5.py`.
This version replaces the iterative dual ascent update for Lagrange multipliers (eta)
with an EXACT adjoint solution at each step, acting as a sanity check and potentially improving convergence.

Key components:
1. Augmented Lagrangian with exact adjoint update for eta.
2. Robust initialization via DAE solve.
3. Feasible path enforcement.
"""
...
def _init():
    parser = argparse.ArgumentParser(description="DAE Parameter Identification (Augmented Lagrangian V5)")
    parser.add_argument(
        '--config', '-c',
        type=str,
...
def example_augmented_lagrangian_v5(config: dict):
    ...
    print("=" * 80)
    print("DAE Parameter Identification with Augmented Lagrangian V5")
    print("=" * 80)
    print("Using: DAEOptimizerAugmentedLagrangianV5 (Exact Adjoint Update)")
    ...
    # Step 4: Optimize
    print("\n" + "=" * 80)
    print("Step 4: Optimize with Augmented Lagrangian V5")
    print("=" * 80)
    
    optimizer = DAEOptimizerV5(
        dae_data_init,
...
if __name__ == "__main__":
    example_augmented_lagrangian_v5(_config)
