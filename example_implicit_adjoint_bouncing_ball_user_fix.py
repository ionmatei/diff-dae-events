...
def run_bouncing_ball_test(config: dict):
    ...
    from src.discrete_adjoint.dae_optimizer_implicit_adjoint_user_fix import DAEOptimizerImplicitAdjoint
    ...
    g_true_val = p_true['g']
    e_true_val = p_true['e']
    g_init = g_true_val * 0.8  # 50% perturbation
    e_init = e_true_val * 0.8   # 50% perturbation
    ...
    for p in dae_data_init['parameters']:
        ...
        if p_name in optimize_params:
            if p_name == 'g':
                p['value'] = g_true_val * 0.8  # 20% perturbation
            elif p_name == 'e':
                p['value'] = e_true_val * 0.8   # 20% perturbation
