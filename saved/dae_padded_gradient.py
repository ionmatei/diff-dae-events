...
class DAEPaddedGradient:
    ...
    def _total_gradient_kernel(W_p, TS_p, p_val, b_types, b_indices, b_param, target_times, target_data, n_targets, t_final, blend_sharpness, funcs, dims, max_blocks, adaptive_horizon=False, soft_interp=False):
        ...
        # 1. Compute Loss Gradients (dL/dW_p, dL/dp, and dL/db_param)
        dL_dW_p, dL_dp, dL_db_param = jax.grad(DAEPaddedGradient._loss_fn_padded, argnums=[0, 1, 5])(
            W_p, p_val, TS_p, b_types, b_indices, b_param,
        ...
        # Note: We pass dL_db_param as an additional input if the sweep supports it.
        # Currently, compute_adjoint_sweep_padded assumes dL_dW contains event time grads 
        # but our W_p doesn't. 
        # Actually, for total gradient consistency, we need the sweep to handle dL/db_param.
        # But wait, dL/db_param is dL/dtau. This is exactly what the adjoint method needs
        # to correctly account for event time sensitivity.
        total_grad = compute_adjoint_sweep_padded(
            W_p, TS_p, p_val,
            b_types, b_indices, b_param,
            dL_dW_p, dL_dp,
            funcs, dims, max_blocks,
            dL_db_param=dL_db_param # We'll need to update the sweep to accept this
        )
        return total_grad
    ...
    def compute_total_gradient(self, sol, p_val, target_times, target_data, blend_sharpness=150.0, adaptive_horizon=False, soft_interp=False):
        """
