...
from debug.verify_residual import prepare_loss_targets
# from debug.dae_matrix_gradient import DAEMatrixGradient
from debug.dae_sparse_block_gradient import DAESparseBlockGradient
...
def run_optimization_test():
    print("=" * 70)
    print("TEST: Adam Optimization using Sparse Block Gradient (Direct Struct)")
    print("=" * 70)
    ...
    # 4. Initialize Sparse Gradient Computer
    grad_computer = DAESparseBlockGradient(dae_data)
    
    # Optimize
    print(f"Adam optimization (Sparse Block): {len(opt_indices)} parameters, max_iter={max_iter}, lr={step_size}")
    ...
    ax1.set_title('Trajectory: Target Data vs Optimized Simulation (Sparse)')
    ax1.legend()
    ...
    plt.tight_layout()
    plot_path = os.path.join(current_dir, 'optimization_result_sparse.png')
