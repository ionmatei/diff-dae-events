"""
Example demonstrating the JAX-based DAE optimizer for parameter identification.

This example uses the fully JIT-compiled optimizer from dae_optimizer_jax.py.
Unlike the original optimizer which uses the IDA solver, this version uses a
JAX-based DAE solver that enables end-to-end JIT compilation.
...
4. Use the JAX optimizer to recover the true parameters for selected parameters only
...
def _init():
    parser = argparse.ArgumentParser(description="JAX-based DAE Parameter Identification")
    parser.add_argument(
        '--config', '-c',
        type=str,
...
def example_parameter_identification_jax(config: dict):
    """Example: Identify DAE parameters using JAX-based optimizer."""

    print("=" * 80)
    print("JAX-based DAE Parameter Identification Example")
    print("=" * 80)
    ...
    # Step 4: Optimize parameters using JAX-based optimizer
    print("\n" + "=" * 80)
    print("Step 4: Optimize Parameters (JAX-based)")
    print("=" * 80)

    # Create JAX optimizer with selected parameters
    optimizer = DAEOptimizerJax(dae_data_init, optimize_params=opt_params)
...
if __name__ == "__main__":
    ...
    print("\n")
    print("RUNNING JAX-BASED PARAMETER IDENTIFICATION EXAMPLE")
    example_parameter_identification_jax(_config)
