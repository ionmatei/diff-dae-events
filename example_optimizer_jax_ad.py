"""
Example demonstrating the JAX-based DAE optimizer using Automatic Differentiation.

This example uses the AD-based optimizer from dae_optimizer_jax_ad.py.
Unlike the adjoint-based optimizer, this version computes gradients directly
through the simulation using JAX's automatic differentiation.
...
4. Use the JAX AD optimizer to recover the true parameters
...
def _init():
    parser = argparse.ArgumentParser(description="JAX AD-based DAE Parameter Identification")
    parser.add_argument(
        '--config', '-c',
        type=str,
...
from src.discrete_adjoint.dae_optimizer_jax_ad import DAEOptimizerJaxAD


def example_parameter_identification_jax_ad(config: dict):
    """Example: Identify DAE parameters using JAX AD-based optimizer."""

    print("=" * 80)
    print("JAX AD-based DAE Parameter Identification Example")
    print("=" * 80)
    ...
    # Step 4: Optimize parameters using JAX AD-based optimizer
    print("\n" + "=" * 80)
    print("Step 4: Optimize Parameters (JAX AD)")
    print("=" * 80)

    # Create JAX AD optimizer with selected parameters
    optimizer = DAEOptimizerJaxAD(dae_data_init, optimize_params=opt_params)
...
if __name__ == "__main__":
    ...
    print("\n")
    print("RUNNING JAX AD-BASED PARAMETER IDENTIFICATION EXAMPLE")
    example_parameter_identification_jax_ad(_config)
