"""
Example demonstrating Augmented Lagrangian V3 for DAE parameter estimation.

This uses the DAEOptimizerAugmentedLagrangianV3 class which implements
Algorithm 3 (Option C) from the documentation.

Key components:
1. Augmented Lagrangian with explicit parameter gradient.
2. Robust initialization via DAE solve.
3. Optimization loop for w (state), eta (multipliers), and theta (parameters).
"""
...
def _init():
    parser = argparse.ArgumentParser(description="DAE Parameter Identification (Augmented Lagrangian V3)")
    parser.add_argument(
        '--config', '-c',
        type=str,
...
from src.augmented_lagrangian.dae_optimizer_augmented_lagrangian_v3 import DAEOptimizerAugmentedLagrangianV3

def example_augmented_lagrangian_v3(config: dict):
    
    print("=" * 80)
    print("DAE Parameter Identification with Augmented Lagrangian V3")
    print("=" * 80)
    print("Using: DAEOptimizerAugmentedLagrangianV3")
    ...
    ncp = 1000 # Use fixed high grid density for reference
    ...
    # Step 4: Optimize
    print("\n" + "=" * 80)
    print("Step 4: Optimize with Augmented Lagrangian V3")
    print("=" * 80)
    
    optimizer = DAEOptimizerAugmentedLagrangianV3(
        dae_data_init,
    ...
    # Set hyperparameters from augmented_lagrangian config section
    ...
    optimizer.n_primal_steps = al_cfg.get('n_primal_steps', 1)
    ...
    print(f"  Max AL iterations: {opt_cfg.get('max_iterations', 100)}")
    ...
    result_opt = optimizer.optimize(
        ...
        y_target=y_target,
        p_init=p_init_opt,
        n_iterations=opt_cfg.get('max_iterations', 50),
...
if __name__ == "__main__":
    example_augmented_lagrangian_v3(_config)
