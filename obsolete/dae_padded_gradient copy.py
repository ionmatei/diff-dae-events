...
class DAEPaddedGradient:
    def __init__(self, dae_data, max_blocks=50, max_pts=500, max_targets=200):
        ...
        self.jit_loss_grad = jax.jit(
            jax.grad(self._loss_fn_padded, argnums=[0, 1]),
            static_argnames=['n_x']
        )
        ...
        self.jit_total_grad = jax.jit(_total_grad_closed)
    ...
    @staticmethod
    def _total_gradient_kernel(W_p, TS_p, p_val, b_types, b_indices, b_param, target_times, target_data, n_targets, t_final, blend_sharpness, funcs, dims, max_blocks):
        """Unified JIT kernel: Loss Differentiation + Adjoint Sweep."""
        n_x, n_z, n_p = dims

        # 1. Compute Loss Gradients (dL/dW_p and dL/dp)
        dL_dW_p, dL_dp = jax.grad(DAEPaddedGradient._loss_fn_padded, argnums=[0, 1])(
            W_p, p_val, TS_p, b_types, b_indices, b_param,
    ...
    def _predict_trajectory_padded_kernel(W_p, TS_p, b_types, b_indices, b_param, target_times, t_final, blend_sharpness, n_x):
        ...
        def _interp_block(ts_row, xs_row, t_c, np_safe):
            """Interpolate one block at one clipped query time."""
            idx = jnp.searchsorted(ts_row, t_c, side='right') - 1
        ...
        def predict_single(t_q):
            ...
            # vmap interpolation over blocks (replaces Python for-loop)
    ...
    @staticmethod
    def _loss_fn_padded(W_p, p, TS_p, b_types, b_indices, b_param, target_times, target_data, n_targets, t_final, blend_sharpness, n_x):
    ...
    def pack_solution(self, sol):
        ...
        Adapted from verify_residual_gmres.py.
        """
        w_list = []
    ...
    def compute_loss_gradients(self, sol, p_opt, target_times, target_data, blend_sharpness=150.0):
        """
        ...
        for kind, count, *extra in structure:
            if kind == 'segment':
                ...
                # The event time parameter gradient is at dL_p[blk_ptr, 0, 0]
                # Wait, jax.grad(loss, W_p) means dL/dt_event is at the event block
                dL_list.append(dL_p[blk_ptr, 0:1, 0:1].flatten())
                blk_ptr += 1
    ...
    def compute_total_gradient(self, sol, p_val, target_times, target_data, blend_sharpness=150.0):
        ...
        # 5. Unified JIT call
        total_grad = self.jit_total_grad(
            W_p, TS_p, p_val,
            b_types, b_indices, b_param,
    ...
    def optimize_adam(self, solver, p_init, opt_param_indices, target_times, target_data,
        ...
        print(f"Adam optimization: {n_opt} parameters, max_iter={max_iter}, lr={step_size}")
        ...
        for it in range(1, max_iter + 1):
            ...
            # --- Compute total gradient (unified JIT kernel) ---
            total_grad = self.compute_total_gradient(
                sol, p_current, target_times, target_data,
            ...
            n_x = self.dims[0]
            loss_val = float(self._loss_fn_padded(
                W_p_d, p_current, TS_p_d, b_types_d, b_indices_d, b_param_d,
                tt_d, td_d, jnp.int32(n_tgt), actual_t_final, blend_sharpness, n_x
            ))
