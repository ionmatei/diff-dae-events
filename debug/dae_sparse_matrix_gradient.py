...
class DAESparseMatrixGradient:
    ...
    def _get_gradient_kernel(self, structure):
        ...
        def compute_grads(W_flat, p_val, grid_taus, target_times, target_data, t_final_val, blend_sharpness, soft_interp):
            ...
            # Use sparse jacobian for J_W (NxN) which is large and sparse (Block Bi-diagonal)
            J_W_func = sparse.jacfwd(residual_fn, argnums=0)
            J_W = J_W_func(W_flat, p_val, grid_taus, t_final_val)
            
            # Use dense jacobian for J_p (NxNp), it's tall and dense-ish
            ...
            # 3. Adjoint Solve
            # Solve J_W.T * lam = -dL_dW
            # J_W is BCOO. Transpose it.
            J_W_T = J_W.transpose()
            
            # Use GMRES
            # Since J is triangular, it should solve easily.
            rhs = -dL_dW
            
            # GMRES requires a linear operator or function. BCOO matvec is supported.
            # Initial guess: zeros
            lam, info = jax.scipy.sparse.linalg.gmres(
                lambda x: J_W_T @ x, 
                rhs, 
                tol=1e-8, 
                maxiter=5000, # Large bound, should converge in 1 step if preconditioned or N steps
                restart=100
            ) 
            # Note: For strict triangular system, GMRES might take N iterations if not preconditioned?
            # Actually, BiCGSTAB might be better for general non-symmetric. 
            # But specific "triangular solve" for sparse is best.
            # JAX doesn't have sparse triangular solve exposed easily yet? 
            # solve_triangular is for dense.
