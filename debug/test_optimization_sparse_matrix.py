...
from debug.dae_sparse_matrix_gradient import DAESparseMatrixGradient
...
def run_optimization_test():
    print("=" * 70)
    print("TEST: Adam Optimization using Sparse Matrix Gradient (JAX Sparse)")
    print("=" * 70)
    ...
    # 4. Initialize Sparse Matrix Gradient Computer
    grad_computer = DAESparseMatrixGradient(dae_data)
    
    # Optimize
    print(f"Adam optimization (Sparse Matrix GMRES): {len(opt_indices)} parameters, max_iter={max_iter}, lr={step_size}")
    ...
    ax1.set_title('Trajectory: Target Data vs Optimized Simulation (Sparse Matrix)')
    ax1.legend()
    ...
    plt.tight_layout()
    plot_path = os.path.join(current_dir, 'optimization_result_sparse_matrix.png')
